...
function RecapPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000016' }}>
        ...
        <div className="photo-screen-content" style={{ justifyContent: 'flex-end', padding: '0 24px calc(48px + env(safe-area-inset-bottom))' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            <div style={{
              ...
              background: 'linear-gradient(135deg, #001FED, #24BBFE)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              margin: '0 auto 16px', fontSize: '2rem', color: 'white',
              boxShadow: '0 8px 32px rgba(107,115,232,0.5)',
            }}>✓</div>

            <h1 style={{ fontFamily: '"Share Tech Mono", monospace', fontWeight: 800, fontSize: '2rem', color: 'white', marginBottom: 6, textShadow: '0 2px 12px rgba(0,0,0,0.4)', letterSpacing: '-0.02em' }}>
      ...
      <div style={{ display: 'flex', background: '#252535', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        {[
          ...
          <div key={i} style={{ flex: 1, textAlign: 'center', padding: '18px 0', borderRight: i < 2 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
            <div style={{ fontFamily: '"Share Tech Mono", monospace', fontSize: '1.9rem', fontWeight: 800, color: 'white' }}>{s.num}</div>
      ...
      {/* Reel */}
      <div id="reel" style={{ background: '#000016' }}>
        {submissions.length === 0 ? (
          ...
          submissions.map((s, i) => (
            <div key={s.id} style={{ padding: '24px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'relative' }}>
              <div style={{ position: 'absolute', top: 24, right: 20, fontFamily: '"Share Tech Mono", monospace', fontSize: '2.5rem', fontWeight: 800, color: 'rgba(255,255,255,0.04)', lineHeight: 1 }}>
                {String(i + 1).padStart(2, '0')}
              </div>
              <div style={{ fontFamily: '"Share Tech Mono", monospace', fontWeight: 800, fontSize: '1.15rem', color: 'white', marginBottom: 2 }}>{s.guests?.name}</div>
      ...
      {/* End card */}
      <div style={{ background: '#000016', textAlign: 'center', padding: '52px 24px 72px' }}>
        <div style={{
        ...
        <h2 style={{ fontFamily: '"Share Tech Mono", monospace', fontWeight: 800, fontSize: '1.5rem', color: 'white', marginBottom: 6 }}>the lore is canon.</h2>
